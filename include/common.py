...
def generate_chunk_array_info(message, peer_id_list):
    """Receive a HELLO or QUERY message and a list chunks ids available in the peer and return a ordered list
    of matches ids"""
    if get_type_of_message(message) == 'HELLO':
        ...
        pointer = 6
    match_chunk_array_info = []
    for i in range(number_of_chunks):
        required_chunk = decode_position(message, pointer + i)
        if required_chunk in peer_id_list:
            match_chunk_array_info.append(required_chunk)
    return sorted(match_chunk_array_info)
